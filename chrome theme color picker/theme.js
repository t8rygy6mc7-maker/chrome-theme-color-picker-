...
(function (global) {
  ...
  const DEFAULT_SETTINGS = {
    ...
    wpShuffle: false, // false = cycle in order, true = random order while rotating
    ...
    weatherSource: "city", // "city" | "geo" — geo uses weatherLat/weatherLon instead
    weatherLat: null, // set only after the user taps "Use my location"
    weatherLon: null,
  };
  ...
  // Wipes the color, settings, and background back to their defaults in one go.
  function resetAll() {
    return new Promise((resolve) => {
      try {
        chrome.storage.sync.set(
          { [STORAGE_KEY]: DEFAULT_COLOR, [SETTINGS_KEY]: Object.assign({}, DEFAULT_SETTINGS) },
          () => {
            try {

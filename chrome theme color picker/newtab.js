(function () {
  ...
  const wpShuffleRow = $("wpShuffleRow");
  const wpShuffleChk = $("wpShuffle");
  ...
  const WALLPAPERS = {
    ...
    animals: [
      ...
      "1474511320723-9a56873867b5",
      "1497752531616-c3afd9760a11",
    ],
    tech: [
  ...
  const Slideshow = {
    ...
    configure(settings) {
      ...
      const sig = active
        ? cat + "|" + intervalMs + "|" + settings.bgWallpaper + "|" + !!settings.wpShuffle
        : "off";
      ...
      this.shuffle = !!settings.wpShuffle;
    ...
    tick() {
      ...
      let nextIndex = (this.index + 1) % this.list.length;
      if (this.shuffle) {
        do {
  ...
  const Weather = {
    ...
    update(settings) {
      ...
      const usingGeo =
        settings.weatherSource === "geo" &&
        typeof settings.weatherLat === "number" &&
        typeof settings.weatherLon === "number";
      ...
      if (!usingGeo && !city) {
        ...
        this.data = null;
        clearTimeout(this.timer);
        this.render({ hint: "Add a city (or tap “Use my location”) in Settings to see the weather." });
        return;
      }

      const key = usingGeo
        ? "geo:" + settings.weatherLat.toFixed(2) + "," + settings.weatherLon.toFixed(2) + "|" + unit
        : city.toLowerCase() + "|" + unit;
      const source = usingGeo ? { lat: settings.weatherLat, lon: settings.weatherLon } : city;
      const stale = Date.now() - this.fetchedAt > this.STALE_MS;
      if (key === this.key) {
        if (this.data && stale) this.load(source, unit, key, usingGeo); // silent refresh
        else if (this.data) this.render(); // fresh cache — just repaint
        return; // (else a request is already in flight)
      }

      // City/location or unit changed — debounce so typing doesn't spam the API.
      this.key = key;
      this.data = null;
      this.render({ loading: true });
      clearTimeout(this.timer);
      this.timer = setTimeout(() => this.load(source, unit, key, usingGeo), 600);
    },

    load(source, unit, key, isGeo) {
      this.key = key;
      const id = ++this.reqId;
      this.render({ loading: true });
      const placePromise = isGeo
        ? Promise.resolve({ lat: source.lat, lon: source.lon, label: "your location" })
        : geocodeCity(source);
      placePromise
        .then((place) =>
        ...
        .catch(() => {
          ...
          this.data = null;
          this.render({
            error: isGeo ? "Couldn't get weather for your location." : "Couldn't find weather for that city.",
          });
        });
    },
  ...
  function syncControls(color, settings, bg) {
    ...
    wpShuffleRow.hidden = (settings.wpRotate || "off") === "off";
    wpShuffleChk.checked = !!settings.wpShuffle;
  ...
  wpShuffleChk.addEventListener("change", (e) => setSettings({ wpShuffle: e.target.checked }));
  uiThemeSeg.querySelectorAll(".seg").forEach((b) => {
  ...
  weatherCityInput.addEventListener("input", (e) =>
    setSettings({ weatherCity: e.target.value, weatherSource: "city" }, true)
  );
  weatherUnitSeg.querySelectorAll(".seg").forEach((b) =>
    b.addEventListener("click", () => setSettings({ weatherUnit: b.dataset.unit }))
  );

  // Opt-in geolocation: only runs on click, only sends coords to Open-Meteo (no key/account).
  function updateGeoBtnLabel() {
    weatherGeoBtn.textContent =
      lastSettings.weatherSource === "geo"
        ? "📍 Using your location — tap to refresh"
        : "📍 Use my location";
  }
  weatherGeoBtn.addEventListener("click", () => {
    if (!("geolocation" in navigator)) {
      weatherGeoBtn.textContent = "Geolocation isn't supported here";
      setTimeout(updateGeoBtnLabel, 2500);
      return;
    }
    weatherGeoBtn.disabled = true;
    weatherGeoBtn.textContent = "Locating…";
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        weatherGeoBtn.disabled = false;
        setSettings({
          weatherSource: "geo",
      ...
      (err) => {
        weatherGeoBtn.disabled = false;
        weatherGeoBtn.textContent =
          err && err.code === err.PERMISSION_DENIED ? "Location permission denied" : "Couldn't get your location";
        setTimeout(updateGeoBtnLabel, 2500);
      },
  ...
  // Reset to defaults — click-again-to-confirm since this wipes shortcuts + background too.
  ...
  resetAllBtn.addEventListener("click", () => {
    ...
    T.resetAll()
      .then(() => refresh({ reloadBg: true }))
      .then(() => {
        resetAllBtn.disabled = false;
        if (!drawer.hidden) renderWallpaperGrid("landscapes");
      });
  });
